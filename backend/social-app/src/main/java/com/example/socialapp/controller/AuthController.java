...
public class AuthController {
  ...
  public record RegisterReq(@NotBlank String username, @Email @NotBlank String email, @NotBlank String password) {}

  @PostMapping("/register")
  public ResponseEntity<?> register(@RequestBody RegisterReq body){
    if (users.existsByUsername(body.username())) return ResponseEntity.badRequest().body(Map.of("error","username_taken"));
    if (users.existsByEmail(body.email()))   return ResponseEntity.badRequest().body(Map.of("error","email_taken"));
    User u = new User();
